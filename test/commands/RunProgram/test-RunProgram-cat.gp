...
# - Run the "cat" program to list a data file to output
...
#RunProgram(CommandLine="type ${WorkingDir}\data\test-file.txt > ${WorkingDir}\expected-results\test-RunProgram-cat-out.txt",UseCommandShell="True")
RunProgram(CommandLine="type ${WorkingDir}\data\test-file.txt > ${WorkingDir}\results\test-RunProgram-cat-out.txt",UseCommandShell="True")
# Compare the results
