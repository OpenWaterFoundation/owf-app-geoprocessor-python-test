...
|`line_shp.zip`|A zipped file. A single feature (`LINE`) Esri shapefile is within the zipped file. Esri shapefile is projected in WGS84 - `EPSG:4326`. Shapefile component files are also in the zipped file (`.cpg`, `.dbf`, `.prj`, `.qgj`, and `.shx`).  |Manually created using QGIS Desktop. Manually archived.| See step-by-step instructions below.|
|`line_shp.zip`|A zipped file. A single feature (`LINE`) GeoJSON file is within the zipped file. GeoJSON is projected in WGS84 - `EPSG:4326`. |Manually created using QGIS Desktop. Manually archived.| See step-by-step instructions below.|
