...
# - because there are no feature classes in the file geodatabase that start with 'C', the test SHOULD FAIL when trying to write the GeoLayer out to a GeoJSON file
#@expectedStatus Failure
# Read all available GeoLayers from the file geodatabase into the GeoProcessor (that start with 'C')
ReadGeoLayersFromFGDB(InputFolder="data/ReadGeoLayersFromFGDB-test.gdb",ReadOnlyOneFeatureClass="False",Subset_Pattern="C*")
