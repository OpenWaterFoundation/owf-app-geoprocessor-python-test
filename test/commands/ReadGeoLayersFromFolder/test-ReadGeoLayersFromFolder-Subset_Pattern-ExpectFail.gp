...
# - because there are no files in the folder that start with 'C', the test SHOULD FAIL when trying to write the GeoLayer out to a GeoJSON file
#@expectedStatus Failure
# Read all available GeoLayers from the data folder into the GeoProcessor (that start with 'C')
ReadGeoLayersFromFolder(InputFolder="data",Subset_Pattern="C*")
